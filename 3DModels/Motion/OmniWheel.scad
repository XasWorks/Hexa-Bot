...
num = 12;	//Number of all subwheels to create. (Has to be a round number!!)
radius = 75/2;	//Radius of the omni-wheel.

//SUBWHEEL VARIABLES
subwheelBaseSize = 5;	//Size of the subwheels at their smallest point (The edge)
subwheelPlay = 1; 		//The additional size that will be cut out around the subwheels to ensure smooth movement
subwheelOverlap = 2;		//Angle in which the subwheel pieces will overlap. Causes smoother movement as there is no definite edge.
...
cRad = subwheelBaseSize - 0.75; 	//Radius of the connectors to the subwheels (Best is slightly smaller than the subwheel.
cThick = 2; 					//Thickness of the connectors.
cPlay = 0.8; 					//Play room for the connector ports (distance to the subwheel)

//FRAME VARIABLES
frameHeight = cRad + subwheelRadius(num, radius);
frameThickness = 17;		//Thickness of the main frame
frameRadius = connectorOffset(num, radius) + frameThickness; //+ cPlay + cThick;		//Radius of the frame Cylinder
...
//CONNECTOR VARIABLES
connectorThickness = 2;	//Thickness of the vertical connectors
connectorInShift = 0.2;		//Inwards shift of the vertical connector beams (for smoother connection)
connectorHeight = 4;		//Height of the connector pieces above the main frame.
...
module centralCylinder(n, r) {
	render(convexity = 4) translate([0,0, - cRad]) difference() {
		union() {
			cylinder(r= frameRadius - frameThickness + cThick + cPlay, h = frameHeight);		//Create the central cylinder.
			//cylinder(r= frameRadius, h = 2*cRad);
			//translate([0, 0, 2*cRad]) cylinder(r1= frameRadius, r2= frameRadius - frameThickness + cThick, h= frameHeight - 2*cRad);
		}
...
module interconnectorPiece(n, r) {
	difference() {
		...
		union() {
			cylinder(r= frameRadius - frameThickness + cThick - 0.1, h= frameHeight);		//Cylinder piece to connect the connector to the main frame.
...
module omniWheel(n, r, subwheels = false) {
	difference() {  union() {
			...
			render(convexity = 5) difference() {
				...
				translate([0,0, -cRad]) cylinder(r= frameRadius - frameThickness - 0.001, h= frameHeight); //Make sure they don't enter the central cylinder
			}

			//Generate the cylinder for the stepper motor axis
			translate([0,0,-cRad]) {
				cylinder(d= 5 + 4, h= frameHeight);
				cylinder(r= frameRadius - frameThickness + cThick + cPlay, h = 3);
			}
...
!subwheelBase(num, radius);
omniWheel(num, radius, true);
translate([0, 0, subwheelRadius(num, radius)*2]) rotate([180, 0, 360/num])
omniWheel(num, radius, true);
